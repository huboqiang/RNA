...
class running_jobs(object):
   ...
   def running_multi(self,cpu=8):
      log_file = "%s.%s" % (  self.sh_work_file, time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime())  )
      shell_work = 'perl /data/Analysis/huboqiang/bin/multi-process.pl -cpu %d   %s >%s' % (cpu,  self.sh_work_file, log_file )
      p = subprocess.Popen(shell_work,shell='True')
      while 1:
         run_cnt = 0
         if p.poll() is None:
